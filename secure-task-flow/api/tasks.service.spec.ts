...
import { AuditAction, TaskCategory, TaskStatus, UserRole } from '@secure-task-flow/data';
...
describe('TasksService', () => {
  ...
  const ownerUser: Partial<User> = {
    id: 1,
    organizationId: 1,
    role: { name: UserRole.OWNER } as any,
  };

  const viewerUser: Partial<User> = {
    id: 2,
    organizationId: 1,
    role: { name: UserRole.VIEWER } as any,
  };
  ...
  describe('findAll', () => {
    ...
    it('should restrict Admin/Viewer to their own org', async () => {
      ...
      const adminUser: Partial<User> = {
        id: 3,
        organizationId: 1,
        role: { name: UserRole.ADMIN } as any,
      };
  ...
  describe('create', () => {
    it('should forbid Viewer from creating tasks', async () => {
      await expect(
        service.create(
          { title: 'x', status: TaskStatus.TODO, category: TaskCategory.WORK, organizationId: 1 },
          viewerUser as User,
        ),
